...
public class Elevator extends Subsystem {
    ...
    public enum ElevatorSetpoint {
        ...
        /*
         * LOWESTHATCHROCKET(0), LOWESTBALLROCKET(0), CARGOHATCH(0), CARGOBALL(0),
         * MIDHATCHROCKET(0), MIDBALLROCKET(0), INITALPOSITION(0); // maybe?
         */
        SCALE(77), // TODO implement above
        SWITCH(30), DEPLOYARMS(20), BOTTOM(0), DONTKNOW(0);
    ...
    // The below refer to the height in inches of various critical points of the
    // elevator
    ...
    private final double k_slopeUp = (k_maxPwrUp - k_finalPwrUp) / k_rampDistUp; // slope of the limit = y / x = power /
                                                                                 // distance
    private final double k_slopeDwn = (k_maxPwrDwn - k_finalPwrDwn) / k_rampDistDwn; // slope of the limit = y / x =
                                                                                     // power / distance
    ...
    public void periodic() {
        ...
        // Zero the encoder if the elevator is at the bottom limit switch and set the
        // elevator initialized flag
        if (atBottomLimit()) {
            if (!m_prevAtBottomLimit) {
                // TODO: UsbLogging.printLog("Elevator encoder held in reset; pre-reset value: "
                // + elevatorEncoder.getDistance());
            }

            // set the elevator encoder to zero so that we can use it to measure elevator
            // height for set points
            // and so that we can ensure that the elevator height does not exceed the
            // limits.
    ...
    public boolean atBottomLimit() {
        // Elevator switch returns true when switch is not pressed because normally
        // open, invert
    ...
    public double getHeight() {
        ...
        // double lidarHeight = (elevatorLidar.getDistance() * k_inPerCm) +
        // k_lidarOffset;

        final double acceptableErrorMargin = 1.0;

        // if(Math.abs(lidarHeight - height) > acceptableErrorMargin)
        // System.out.println("LIDAR and Encoder not in agreement within specified
        // margin of error");
    ...
    public double limitMotorPwr(double pwr, boolean override) {
        ...
        // max power limit
        if (pwr > k_maxPwrUp)
            pwr = k_maxPwrUp; // Never exceed the max Up power
        if (pwr < k_maxPwrDwn)
            pwr = k_maxPwrDwn; // Never exceed the max Down power
        ...
        if (override == false) {
            ...
            if (atTopLimit() && (pwr > 0))
                pwr = 0; // Do not go up
            if (atBottomLimit() && (pwr < 0))
                pwr = 0; // Do not go down
            ...
            if (isHeightValid()) {
                ...
                upLimit = k_finalPwrUp + (distance * k_slopeUp); // calculating upLimit, could much greater than
                                                                 // maxPwrUp
                ...
                dnLimit = k_finalPwrDwn + (distance * k_slopeDwn); // calculating dnLimit, could much less than
                                                                   // maxPwrDwn
